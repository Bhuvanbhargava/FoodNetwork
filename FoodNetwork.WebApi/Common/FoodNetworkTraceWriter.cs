...
{
    public class FoodNetworkTraceWriter : ITraceWriter
    {
        ...
        public FoodNetworkTraceWriter() 
        {
            // Setup the default pre-filter to accept only trace requests that are:
            //  1. Warn, Error or Critical level (any level above or equal warning), or
        ...
        public void Trace(System.Net.Http.HttpRequestMessage request, string category, TraceLevel level, Action<TraceRecord> traceAction)
        {
            ...
            if (level >= TraceLevel.Error)
            {
                ...
                FoodnetworkEventSource.Log.Error(
                    record.Operator,
                    record.Operation,
            ...
            else if (level == TraceLevel.Warn)
            {
                ...
                FoodnetworkEventSource.Log.Warning(
                    record.Operator,
                    record.Operation,
                    record.Exception != null ? record.Exception.ToString() : record.Message);
            }
            else if (category == TraceCategories.RequestCategory)
            {
                ...
                {
                    FoodnetworkEventSource.Log.Request(
                ...
                else
                {
                    ...
                    FoodnetworkEventSource.Log.Response(
                        request.RequestUri != null ? request.RequestUri.ToString() : string.Empty,
                        request.Method.ToString(),
                        (int)record.Status,
            ...
            else
            {
                ...
                switch (record.Kind)
                {
                    case TraceKind.Begin:
                        FoodnetworkEventSource.Log.OpBegin(
                    ...
                    case TraceKind.End:
                        FoodnetworkEventSource.Log.OpEnd(
                            record.Operator ?? String.Empty,
                            record.Operation ?? String.Empty,
                            record.Message ?? String.Empty
                            );
                    ...
                    default:
                        FoodnetworkEventSource.Log.OpTrace(
                            record.Operator ?? String.Empty,
                            record.Operation ?? String.Empty,
                            record.Message ?? String.Empty
                            );
            ...
            if (httpResponseException != null)
            {
                ...
                if (objectContent != null)
                {
                    ...
                    if (httpError != null)
                    {
                        ...
                        {
                            messages.Add(String.Format(EventSourceResources.HttpErrorUserMessageFormat, messageObject));
                        }

                        if (httpError.TryGetValue(MessageDetailKey, out messageDetailsObject))
                        {
                            messages.Add(String.Format(EventSourceResources.HttpErrorMessageDetailFormat, messageDetailsObject));
                        }
